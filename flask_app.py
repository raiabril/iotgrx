...
from query import get_sensors_query, get_events_query, get_devices_query
...
def add_device():
    ...
    json_object = Device(**posted_object.data, created_by="HTTP post request")
    ...
    # return created exam
    new_object = DeviceSchema().dump(json_object).data
    session.close()
...
def add_sensor():
    ...
    for dat in data:
        ...
        json_object = Sensor(**posted_object.data, created_by="HTTP post request")
        session = Session()
        session.add(json_object)
        session.commit()
        new_object_list.append(SensorSchema().dump(json_object).data)
        session.close()
...
def add_events():
    ...
    for dat in data:
        ...
        json_object = Event(**posted_object.data, created_by="HTTP post request")
        session = Session()
        session.add(json_object)
        session.commit()
        new_object_list.append(EventSchema().dump(json_object).data)
        session.close()
